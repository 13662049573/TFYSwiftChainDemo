//
//  TFYEnhancedRuntimeChainSystem.swift
//  TFYSwiftChainDemo
//
...
public struct TFYChain<T: NSObject>: TFYChainableProtocol, TFYChainErrorHandling, TFYChainPerformanceMonitoring, TFYChainAdvancedFeatures {
    ...
    /// - Parameters:
    ///   - duration: 动画时长
    ///   - options: 动画选项 (在macOS上被忽略，使用默认动画设置)
    ///   - animations: 动画闭包
    ///   - completion: 完成回调
    /// - Returns: 链式容器
    ...
    /// - Parameters:
    ///   - duration: 动画时长  
    ///   - damping: 阻尼比 (在macOS上被忽略，使用easeInEaseOut时间函数)
    ///   - velocity: 初始速度 (在macOS上被忽略)
    ///   - options: 动画选项 (在macOS上被忽略)
    ///   - animations: 动画闭包
    ///   - completion: 完成回调
    /// - Returns: 链式容器
...
extension TFYChain {
    ...
    private func performPropertySetting(propertyName: String, value: Any, chain: inout TFYChain<T>) -> Bool {
        ...
        if let activityIndicator = base as? UIActivityIndicatorView {
            switch propertyName {
            case "startAnimating":
                if let shouldStart = value as? Bool {
                    if shouldStart {
        ...
        // iOS 特有的搜索栏特殊属性
        if let searchBar = base as? UISearchBar {
            switch propertyName {
            case "searchText":
        ...
        // iOS 特有的页面控制器特殊属性
        if let pageControl = base as? UIPageControl {
            switch propertyName {
            case "numberOfPages":
